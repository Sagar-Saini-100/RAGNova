...
def _print_chunk_summary(chunks) -> None:
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        src = meta.get("source_path") or "<no path>"
        ...
        if chunk.content:
            ...
            ellipsis = "…" if len(chunk.content) > 120 else ""
...
def main() -> None:
    ...
    if chunks:
        first = store.get(chunks[0].id)
        print(f"store.get('{chunks[0].id}') → Chunk(modality={first.modality!r})\n")
...
def programmatic_example(image_folder: str) -> None:
    ...
    for chunk in chunks:
        ...
        print(f"  ocr_text       : {chunk.content[:80]!r}")
    ...
    if chunks:
        retrieved = store.get(chunks[0].id)
        assert retrieved is not None
        print(f"Retrieved from store: {retrieved.id}")
